...
const SERVICIOS = [
  { key: 'usuarios_activos_pct',    label: 'Acueducto',      fill: '#1d4ed8' },
  { key: 'alc_usuarios_activos_pct',label: 'Alcantarillado', fill: '#0891b2' },
  { key: 'en_usuarios_activos_pct', label: 'Energía',        fill: '#059669' },
];
...
export default function GraficaUsuariosServicios({ datos, titulo, subtitulo, nota }) {
  const series = datos
    .filter((d) => d.municipio && SERVICIOS.some((s) => d[s.key] != null))
    .map((d) => ({
      municipio: d.municipio?.length > 10 ? d.municipio.substring(0, 10) + '…' : d.municipio,
      municipioFull: d.municipio,
      ...Object.fromEntries(SERVICIOS.map((s) => [s.key, d[s.key] != null ? Number(d[s.key]) : null])),
    }));
    ...
    return (
      <div className="bg-white rounded-xl border border-slate-200 shadow-sm p-5">
        ...
        <p className="text-slate-400 text-sm py-8 text-center">
          Sin datos de usuarios para el año seleccionado. Disponible a partir de 2020 para algunos municipios.
        </p>
      </div>
    );
  }
    ...
    <div className="bg-white rounded-xl border border-slate-200 shadow-sm p-5">
      ...
      <ResponsiveContainer width="100%" height={280}>
        <BarChart data={series} margin={{ top: 10, right: 20, left: 0, bottom: 40 }}>
          ...
          <XAxis dataKey="municipio" tick={{ fontSize: 11, fill: '#64748b' }} angle={-35} textAnchor="end" interval={0} />
          <YAxis tick={{ fontSize: 12, fill: '#64748b' }} tickFormatter={(v) => `${v}%`} domain={[0, 100]} />
          <Tooltip content={<TooltipPersonalizado nota={nota} />} />
          <Legend wrapperStyle={{ fontSize: 12, paddingTop: 12 }} />
          {SERVICIOS.map((s) => (
            <Bar key={s.key} dataKey={s.key} name={s.label} fill={s.fill} radius={[3, 3, 0, 0]} />
          ))}
        </BarChart>
      </ResponsiveContainer>
    </div>
